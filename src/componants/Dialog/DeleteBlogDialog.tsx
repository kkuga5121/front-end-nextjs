...
interface DialogBlogDeleteProps {
  isOpen: boolean;
  onClose: () => void;
  blog: IBlog | null;
}
const DeleteBlogDialog: React.FC<DialogBlogDeleteProps> = ({
  isOpen,
  onClose,
  blog,
}) => {
      ...
      try {
        ...
        if (blog) {
          ...
          if (blog.userId == user.id) {
            responce = await deleteBlogPost(blog.id);
          } else {
            responce = { message: "Can not Delete this Blog." };
          }
        }
        if (responce.data) {
